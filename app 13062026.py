...
CQS = {
    ...
    "CQ8": {
        "challenge": "Does any concurrent action remove a condition that must remain continuously true during this action's execution? (Distinct from CQ6: CQ6 concerns whether concurrent actions can legally begin; CQ8 concerns whether a concurrent action destroys a continuously required condition.)",
        ...
        "how": "S5 Premise 3 directly certifies mutual invariant compatibility: no invariant condition required by one concurrent action is removed by any effect of the other during their shared window. This is the most direct possible answer to CQ8.",
...
def evaluate_cqs(action, scenario_key):
    ...
    for cq_id, cq in CQS.items():
        if action["inv"] == [] and cq_id in ("CQ2","CQ8"):
            results[cq_id] = "n/a"
        elif not action["concurrent_with"] and cq_id in ("CQ6","CQ8"):
            results[cq_id] = "n/a"
...
st.sidebar.image("https://www.hud.ac.uk/media/assets/img/logos/logo-hud.svg",
                 width=180)
...
# ══════════════════════════════════════════════════════════════
    ...
    with col1:
        ...
        defeat_map = [
            ...
            {"CQ":"CQ7","Attacks":"S6 (P3)","Defeated by":"S2 (P2–P3)"},
            {"CQ":"CQ8","Attacks":"S7 (P2)","Defeated by":"S5 (P3)"},
        ]
...
- **Out:** all CQ1–CQ8 arguments (all defeated)
- **Undecided:** none
            """)
        else:
